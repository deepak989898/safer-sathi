...
async function executeTripJackHotelBook(
  ...
  if (bookRejected) {
    const failed = await updateHotelBooking(bookingId, {
      ...
      bookRequest,
      bookResponse,
      rawTripjackBookResponse: bookResponse,
      normalizedTripjackStatus: orderStatus || "FAILED",
  ...
  const updated = await updateHotelBooking(bookingId, {
    ...
    bookResponse,
    rawTripjackBookResponse: bookResponse,
    normalizedTripjackStatus: (normalized?.orderStatus ?? "").toUpperCase() || (confirmed ? "SUCCESS" : "PENDING"),
    tripjackBookingId: normalized?.bookingId || booking.tripjackBookingId,
    supplierBookingId: normalized?.bookingId || booking.tripjackBookingId,
    supplierReference: normalized?.supplierReference,
    hotelReference:
      normalized?.supplierReference || normalized?.confirmationNumber || normalized?.bookingId || booking.tripjackBookingId,
