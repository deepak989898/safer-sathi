...
import { checkBookedTicket } from "@/lib/seatseller/client";
...
export async function POST(request: Request) {
  try {
    ...
    if (parsed.data.bookingId) {
      ...
      if (!booking?.tin) return apiError("TIN not found", 404);
      tin = booking.tin;
    }
    if (!tin) return apiError("tin or bookingId required", 400);

    const status = await checkBookedTicket(tin, parsed.data.bookingId);
    return apiSuccess({ status });
  } catch (error) {
    return busApiError(error, "Failed to check ticket status");
  }
}
